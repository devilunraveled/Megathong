...
if __name__ == "__main__":
    ...
    texts = dataset.sample(batch_size)["post"].values

    # Define the prompt template for each text
    prompts = [
        f"""Classify the text into {','.join(RedditData.labels)} and return the answer as the corresponding mental health disorder label.
        text: {text}
        label: """.strip() for text in texts
    ]
